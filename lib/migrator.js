 ...
 */
function Migrator(compound, schemaType, migrationsPath) {
    ...
    this.schema   = schemaType === null ? compound.orm.schema[0] : compound.orm.schema[schemaType];
    return this;
}
...
Migrator.prototype.runMigrations = function (toVersion, direction, done) {
    ...
    var queue = [];
    this.compound.models.Migration.maxVersion(function (err, currentVersion) {
        ...
        var files = this.getMigrations(direction, currentVersion, toVersion, this.path);
       ...
       this.setMigrationsActiveState(active, direction, toVersion, function(error){
            if (error) { done(error); }
            var migrator = this;
            function runMig(mig) {
                if (mig) {
...
Migrator.prototype.updateMigrationActiveState = function(toVersion, mig, done ) {
    if (toVersion) {
        if (semver.gte(mig.version, toVersion)) {
...
Migrator.prototype.setMigrationsActiveState = function (active, direction, toVersion, done) {
    if (active === false && direction == 'down') {
        this.compound.models.Migration.all({where: {active: true}}, function(err, migrations){
            if (err) { done(err); }
            if (migrations.length > 0) {
                migrations.forEach(function (mig, index){
                    this.updateMigrationActiveState(toVersion, mig, function(err){
                        if (err) { done(err); }
 ...
 */
Migrator.prototype.getMigrations = function (direction,
                                             fromVersion,
                                             toVersion,
                                             migrationsPath,
                                             relativeFolder) {
    ...
    for (var i in dirContents) {
        ...
        if (stats.isFile()) {
            ...
            var childFiles = this.getMigrations(direction,
                                                fromVersion,
                                                toVersion,
                                                itemPath,
    ...
    if ((direction !== null && direction == 'down') && (fromVersion === null)) {
        return [];
    } else {
        return files
            .sort(byVersion)
...
Migrator.prototype.maxMigration = function () {
    var migrations = this.getMigrations(null, null, null, this.path);
